...
def create_menu():
    ...
    left_frame = ctk.CTkScrollableFrame(root_frame, width=300)
    ...
    social_networks = [
        {"name": "YouTube", "filename": "youtube.png", "options": ["Agendar Vídeo", "Gerenciar Playlist"]},
        {"name": "Facebook", "filename": "facebook.png", "options": ["Criar Post", "Análise de Dados"]},
        {"name": "Twitter / X", "filename": "x.png", "options": ["Agendar Tweet", "Análise de Engajamento"]},
        {"name": "Instagram", "filename": "instagram.png", "options": ["Agendar Post", "Gerenciar Stories"]},
    ...
    def toggle_network(network, switch_var, options_frame):
        """Alterna o estado de seleção de uma rede social e exibe/esconde opções."""
        if switch_var.get():
            if network not in selected_networks:
                selected_networks[network] = True
            options_frame.pack(fill="x", padx=20, pady=5)

            if network == "YouTube":
                youtube_interface(right_frame)  # Chamada direta à interface do YouTube
        else:
            if network in selected_networks:
                del selected_networks[network]
            options_frame.pack_forget()

            # Limpa o lado direito quando a seleção é removida
            for widget in right_frame.winfo_children():
                widget.destroy()

        update_selected_networks()

    def update_selected_networks():
        """Atualiza a exibição das redes sociais selecionadas no lado direito."""
        for widget in right_frame.winfo_children():
            widget.destroy()

        if selected_networks:
            for network in selected_networks.keys():
                icon_path = next((n['filename'] for n in social_networks if n['name'] == network), None)
                if icon_path:
                    icon_full_path = os.path.join(icon_dir, icon_path)
                    if os.path.exists(icon_full_path):
                        icon_image = ctk.CTkImage(light_image=Image.open(icon_full_path), size=(50, 50))
                        ...
                        ctk.CTkLabel(item_frame, text=network, font=("Arial", 14)).pack(side="left", padx=10)

            publish_button = ctk.CTkButton(right_frame, text="Publish", font=("Arial", 16), command=publish_selected)
            publish_button.pack(pady=20)

    def publish_selected():
        """Publica nas redes sociais selecionadas."""
        print(f"Publicando nas redes sociais: {', '.join(selected_networks.keys())}")
    ...
    for network in social_networks:
        ...
        for option in network["options"]:
            ctk.CTkLabel(options_frame, text=option, font=("Arial", 12)).pack(anchor="w", pady=2)

        switch = ctk.CTkSwitch(item_frame, text=network["name"], variable=switch_var,
                               command=lambda n=network["name"], sv=switch_var, of=options_frame: toggle_network(n, sv, of))
        switch.pack(side="left", fill="x", expand=True, padx=10)
